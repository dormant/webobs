...
# Test Radian stations
# RCS, 15-Jul-2019
my @radistations;
my @radistations_mon = qw( MBFL_HHZ_MV MBGH_HHZ_MV );

# Test Turkey stations
# RCS, 24-Jul-2019
my @turkstations;
my @turkstations_mon = qw( TRC1_HHZ_TK TRC2_HHZ_TK TRC3_HHZ_TK );

my @dates = ();
my @whats = qw( heli sgram pan );

my $web_start_page = '/cgi-bin/seismic_plot_viewer_test.cgi';
...
if( $what_to_plot eq 'heli' ) {
	...
	@radistations = @radistations_mon;
	@turkstations = @turkstations_mon;
}
...
print "<BR>\n";
print "<B>MVO Radians:</B>", $spaces, "\n";

	foreach my $radistation ( @radistations ) {
		my ($radistation_nice,$dum1,$dum2) = split /_/, $radistation;
		my $link = sprintf( "%s?station=%s&date=%s&what=%s&layout=%s", 
			$web_start_page, $radistation, $date_to_plot, $what_to_plot, $layout );
		#print "$station\n";
		#print "$station_to_plot\n";
		if( $radistation eq $station_to_plot) {
			print $markerl;
		} else {
			print $markersp;
		}
		print $q->a({href=>$link},$radistation_nice);
		if( $radistation eq $station_to_plot) {
			print $markerr, $spaces, $spaces;
		} else {
			print $markersp, $spaces, $spaces;
...
print "<BR>\n";
print "<B>Erciyes Dag:</B>", $spaces, "\n";

	foreach my $turkstation ( @turkstations ) {
		my ($turkstation_nice,$dum1,$dum2) = split /_/, $turkstation;
		my $link = sprintf( "%s?station=%s&date=%s&what=%s&layout=%s", 
			$web_start_page, $turkstation, $date_to_plot, $what_to_plot, $layout );
		#print "$station\n";
		#print "$station_to_plot\n";
		if( $turkstation eq $station_to_plot) {
			print $markerl;
		} else {
			print $markersp;
		}
		print $q->a({href=>$link},$turkstation_nice);
		if( $turkstation eq $station_to_plot) {
			print $markerr, $spaces, $spaces;
		} else {
			print $markersp, $spaces, $spaces;
